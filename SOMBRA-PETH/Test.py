...
class PETH():
    ...
    def export_to_sql(self):
        ...
        try:
            ...
            print(f'File Exported: {self.match_id}, {self.map}')
        except exc.IntegrityError as e:
            print(f"Integrityerror: {e}")

if __name__ == "__main__":
    starttime = time.time()
    db_name = 'New York Excelsior'
    table_name = 'finalstat'
    sql = f'select distinct MatchId, Map from {table_name};'
    df_init = pd.read_sql(sql = sql, con = MySQLConnection(dbname = db_name).engine)
    ...
    print(f"[{count} rows x {length} columns]")
