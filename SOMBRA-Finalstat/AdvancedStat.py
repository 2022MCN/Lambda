...
class RCPv1(AdvancedStat):
    ...
    '''
    def __init__(self, team_one_name, input_df=None):
    ...
    def define_df_stat(self):
        ...
        team_one_NumAlive = df_init[df_init['Team'] == self.team_one_name]
        team_one_NumAlive = team_one_NumAlive.groupby(by=self.idx_col).max()

        # team_two_NumAlive (Opponent)
        team_two_NumAlive = df_init[df_init['Team'] != self.team_one_name]
